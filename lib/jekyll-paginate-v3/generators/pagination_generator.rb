...
class PaginationGenerator < Jekyll::Generator
	...
	def generate(site)
		...
		logger.debug("Normalised config summary: enabled=#{config['enabled']} compatibility=#{config['compatibility'] || 'none'} items=#{config.dig('templates', 'items')} templates.location=#{config.dig('templates', 'location')} generate.count=#{config.dig('templates', 'generate')&.length || 0}.")
